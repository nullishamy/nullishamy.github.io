...
{#if depth === 1}
	<h1 {id} class="flex flex-row items-center justify-items-center gap-6">
		<a
			href="#{id}"
			aria-label="Link to the {id} section of the article"
			class="text-sky min-h-15 min-w-15"><Icon icon="ph:hash" /></a
		><slot></slot>
	</h1>
{:else if depth === 2}
	<h2 {id} class="flex flex-row items-center justify-items-center gap-6">
		<a href="#{id}" aria-label="Link to the {id} section of the article" class="text-sky"
			><Icon icon="ph:hash" /></a
		><slot></slot>
	</h2>
{:else if depth === 3}
