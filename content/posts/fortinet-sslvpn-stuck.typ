...
#html.img(
    src: "posts/fortinet-ssl-vpn-stuck/vpn-architecture.png",
    alt: "a diagram showing the architecture of the vpn, as described above, in picture form. it shows the traffic flowing through the firewall to an internal server",
    title: "traffic flowing from a client device, to the firewall, and to an internal server"
)
...
In my case I _had_ to mess with the registry because the MSI was removed enough that no standard procedure could clean it up. The uninstall dialogs complained about missing files (#link("https://stackoverflow.com/questions/334490/uninstall-without-an-msi-file")[this StackOverflow question] goes over it), and `msiexec` simply said the product wasn't installed.
