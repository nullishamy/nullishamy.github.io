...
This post was migrated from my Ghost blog instance. Whilst accurate in its reporting, it is not _up to date_
...
#quote[#text("@everyone steam gift 50$ - [steamcommunity.com/gift-card/pay/50](<bad domain>)")]
#quote[#text("steam gives a personal gift - [steamcommunity.com/gifts/id=1131341079](<bad domain>)")]
#quote[#text("50$ gift - [steamcommunity.com/105389195](<bad domain>)")]
...
#html.img(
    src: "/posts/moderation/discord-click-warn.png",
    alt: "the dialog box informing the user that a known bad domain was clicked, forbidding them from continuing",
    title: "the dialog box informing the user that a known bad domain was clicked, forbidding them from continuing",
    style: "width: 75%; margin: auto;"
)

3) Though I've hidden them here to avoid _accidents_, lots of the destinations are URL shorteners, often based in Russia, and often being short lived. In the team we jokingly call them YARLS (Yet Another Russian Link Shortener) because of how often we see new ones popping up.
...
An interesting side effect of this rotation tactic is that the attackers have to resort to increasingly odd domain names which are close to "steamcommunity.com" (the real domain for Steam). One example of this can be seen in #link("https://radar.cloudflare.com/scan/f064bd20-c33f-44ff-b5ed-3a1991d3c100/summary")[this scan report] where Radar classifies the domain as "DGA" (Domain Generation Algorithm), which would be a much more reliable heuristic to match against.
...
#html.img(
    src: "/posts/moderation/steam-scam-site.png",
    alt: "a screenshot of a steam scam page, featuring a lure for a 50\$ 'free credit'",
    title: "a screenshot of a steam scam page, featuring a lure for a 50\$ 'free credit'",
    style: "width: 75%; margin: auto;"
)
...
In TCD, we have a #link("https://docs.thecodingden.net/community-policy-center/faq/moderation#why-did-my-message-get-blocked")[blanket ban] on the masked links described above, with a small timeout applied to reduce load on other systems:
...
#html.img(
    src: "/posts/moderation/security-actions.png",
    alt: "The 'Security Actions' pane showing the ability to pause invites and pause DMs between members who share the server but are not friends",
    title: "the security actions pane",
    style: "width: 75%; margin: auto;"
)

In terms of our own defences, we use #link("https://beemo.gg/")[Beemo] which works well.. when it activates. Beemo is very conservative due to its public nature, so will generally only trip for big raids, but does great work when it finds a raid.
