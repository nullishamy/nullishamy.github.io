...
#let args = (
    ...
    date: parse-date("2025-09-02"),
    author: "amy erskine",
    summary: [managing mail defaults],
    tags: ("ibm", "windows"),
)
...
As it turns out, the user was not clicking on a mailto link, an eml file, or anything typical. They were using the explorer _context action_ to initiate their email.
...
The #link("https://en.wikipedia.org/wiki/MAPI")[MAPI protocol] is what allows programs to be "email aware". The actual calling process for MAPI involves looking for the DLL specified at the registry path `HKLM\SOFTWARE\Clients\Mail\{client}\DLLPath[Ex]`, which will contain the exposed functions required for MAPI
