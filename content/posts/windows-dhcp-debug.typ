...
This was compounded by an oversight in our #link("https://networkengineering.stackexchange.com/questions/41376/how-ip-helper-address-works")[IP-helper] setup, which was sending all DHCP requests to a single DC. This is critical to check if you are trying to debug DHCP (explored soon), as you won't see any traffic in Wireshark traces if you aren't looking where the requests actually end up!
...
#html.img(
    src: "/posts/windows-dhcp-debug/dhcp-proto.png",
    alt: "a diagram showing the normal flow for DHCP. it shows the DISCOVER, OFFER, REQUEST, and ACKNOWLEDGE communication",
    title: "the flow of DHCP when it is working as intended",
